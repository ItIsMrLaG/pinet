...
const IHeap = @import("memory.zig").IHeap;
const BasicHeap = @import("memory.zig").BasicHeap;
...
pub const Name = struct {
    ...
    pub fn unchain(name: *Name, heap: IHeap(Name)) void {
...
test "unchain" {
    ...
    var basic_name_heap: BasicHeap(Name) = try .init(gpa, 20);
    defer basic_name_heap.deinit(gpa);

    var basic_agent_heap: BasicHeap(Agent) = try .init(gpa, 20);
    ...
    try std.testing.expectEqual(.free, @as(*BasicHeap(Name).Optional, @fieldParentPtr("item", b)).*);
    try std.testing.expectEqual(.free, @as(*BasicHeap(Name).Optional, @fieldParentPtr("item", c)).*);
